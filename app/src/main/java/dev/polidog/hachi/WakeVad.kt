...
class WakeVad(
    ...
    /**
     * How loud a 10 ms frame has to be to count as speech, on the int16 scale.
     *
     * [RMS_THRESHOLD] is butler's value, derived from butler's recording on butler's audio source.
     * The same room through a different gain is a different number, and a value set too high is
     * silent in exactly the way a broken microphone is, so this is a knob rather than a constant --
     * see the level line WakeWord logs, and `wakeLevel` in Settings.
     */
    private val rmsThreshold: Double = RMS_THRESHOLD,
) {
    /**
     * The loudest frame seen since it was last read, and how many segments have been opened.
     *
     * Read and reset by the owner once a second to log what the microphone is actually delivering.
     * Written from the one thread that calls [accept], read from that same thread.
     */
    ...
    private fun processFrame() {
        ...
        if (!inSpeech) {
            if (active) {
                inSpeech = true; hangoverLeft = HANGOVER_FRAMES; segmentFrames = 1; segments++
                onSegmentStart()
                flushPreRoll()
